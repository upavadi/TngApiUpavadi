...
class TngApiCustom_Shortcode_DanniversariesCustom extends Upavadi_Shortcode_AbstractCustomShortcode
{
    const SHORTCODE = 'tngcustom_pages_danniversariescustom';
   ...
   public function show()
    {
		...
		$currentDay = date('d');
		$currentMonth= date('m');
        $danniversariescustom = $this->getCustomDeathAnniversaries($month);
        $url = $this->content->getTngUrl();
		$date = new DateTime();
        $date->setDate($year, $month, 01);	
		$photos = $this->content->getTngPhotoFolder();
		$photosPath = $url. $photos;$currentperson = $this->content->getCurrentPersonId($person['personID']);
        $context = array(
            'year' => $year,
			'month' => $month,
			'currentDay' => $currentDay,
			'currentMonth' => $currentMonth,
            'danniversariescustom' => $danniversariescustom,
			'gedcom' => $tree,
		...
		return $this->templates->render('d-anniverseriescustom.html', $context);
    }
	public function getCustomDeathAnniversaries($month)
    {
